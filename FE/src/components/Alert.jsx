...
export default function Alert({
  ...
  const showCancel = hidden !== "hidden";

  return (
    <>
      {show && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center">
          <div className="w-200 bg-white h-2/3 rounded-2xl shadow-sm flex flex-col items-center justify-center gap-20">
            <button
              onClick={onClose}
                ...
                <div className="flex items-center justify-center gap-20 text-sm">
                  {showCancel && (
                    <Button
                      ...
                      className="border-1 border-[#D8D6DE] rounded-sm w-30 h-10 hover:border-black"
                    >
                      {Cancel}
                    </Button>
                  )}
